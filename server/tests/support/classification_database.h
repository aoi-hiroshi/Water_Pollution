...
public:
    ...
    ExecuteResult execute(std::string_view sql,std::span<const DbValue> params) override {
        ...
        if(!sql.starts_with("INSERT INTO classification_")) throw std::runtime_error("raw mutation forbidden");
        if(sql.find("cleaning_runs")!=sql.npos) {
            if(params.size()!=8 || std::get<std::int64_t>(params[0])!=1) throw std::runtime_error("bad run params");
            const auto run=state_->next_run++; state_->datasets[run]=std::get<std::string>(params[1]);
            return {1,static_cast<std::uint64_t>(run)};
        }
        if(state_->fail_write) throw DbError("injected save failure");
        if(params.size()%23!=0 || params.size()>128*23) throw std::runtime_error("bad batch size");
        for(std::size_t start=0;start<params.size();start+=23) {
    ...
    DbRows query(std::string_view sql,std::span<const DbValue> params) override {
        ...
        if(sql.find("FROM classification_cleaning_runs")!=sql.npos) {
        ...
        if(sql.find("FROM classification_cleaned_samples")!=sql.npos) {
            const auto run=std::get<std::int64_t>(params[0]);
